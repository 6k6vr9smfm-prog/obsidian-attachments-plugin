...
- [X] 0.7.2 — fix iOS preview generation on the import path. `vault.readBinary(tfile)` right after `vault.createBinary(tfile)` was returning empty/stale data on Capacitor's async filesystem before Obsidian re-indexed the fresh file. Switched the preview adapter's read path to `vault.adapter.readBinary(path)` (low-level, race-free). The desktop reactive and "Generate missing previews" paths were unaffected because their TFiles were already indexed; the bug only surfaced when a file was read in the same microtask as the write that created it.
